...
| Model-boundary scans not yet run live | `HiddenLayerScanner` speaks the v2 interaction-evaluations API, LIVE-VERIFIED at the ingestion boundary (2026-07-18: the poisoned sample essay returns DETECT `[System] Prompt Injection` HIGH and is refused fail-closed mid-heartbeat; clean essays return NONE). The per-request model prompt/output scans (`ScannedModelClient`) are offline-tested but have not run against a live model endpoint yet — that requires the vLLM/Featherless route. | Bring up the model endpoint (`MODEL_ADAPTER=openai`) and run `heartbeat:essays`; the prompt/output scans ride the same verified scanner.                              |
