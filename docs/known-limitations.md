...
| "Live" feed is a fixture replay | `FixtureSourceAdapter` replays a synthetic JSON feed. Freshness is real (metrics change between polls) but the data is not.                                                                             | Implement `LiveSourceAdapter.todo.ts` against a real public source. See [dataset-provenance.md](dataset-provenance.md).                                               |
...
2. Swap the **writer** and **researcher** for Nemotron/vLLM-served models.
3. Implement a **real live feed** via `LiveSourceAdapter`.
